...
| `housellm` | `format` → `dedup` → `toon` → `cmdfilter` → `searchfold` → `textclean` → `extract_llm` → `extract` → `cachesplit` → `toolfilter` | `house` plus the compaction-model pass, applied per account on request. `per_output: false`, so it runs **only on a TTL-expiry sweep** and never on prompt-cached traffic. `allow_on_caching_backend: true` (the only preset that sets it) is therefore **inert as shipped** — the check it lifts is unreachable, because the cold path prices an expired prefix as uncached. It is set so that turning `per_output` on is a deliberate act rather than a silent one; that combination is net-negative on our own measurements and a guard test forbids it in any default. `llm_max_per_session: 0` and `llm_max_per_request: 4` are likewise never consulted while `per_output` is off (they are the other arm of the same if/else). The bounds that actually apply: **`cold_cache.max_calls: 20`** per sweep, the economic gate, and a 20,000-token pressure trigger. |
