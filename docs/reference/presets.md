...
| `agent` | `format` → `dedup` → `failed_run` → `mask` → `extract` → `extract_llm` → `cachesplit` | Long agentic sessions (e.g. Claude Code on SWE-bench) where re-sent tool outputs dominate cost. **`extract_llm` is the biggest lever** — ~27% content-token savings with no task-reward loss (see [Benchmarks](../RESULTS.md)); that figure was previously miscredited to `mask`, which is not even present in the arm that produced it and is [structurally inert on caching traffic](../components/mask.md#when-its-inert). |
