...
The folder is safe to delete once you're sure you no longer need to recover anything through it — a
clean `/context-guru:uninstall` doesn't remove it automatically, on the theory that "I might still
want the original back" outlives "I just undid the install." If you delete it, you also give up the
`context-guru-reset` escape hatch for that file: with no `.pre-install` copy, there is nothing to
restore from and the hatch will say so honestly rather than pretending it can help.
