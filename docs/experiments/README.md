...
| [loca/iter011](loca/iter011/results.md) | 2026-08-21/22 | Deferral: `summarize` vs `+coref` vs `+fold`, 32k, n=75/arm | **Deferral confirmed and monotonic** — `summarize` fires on 56.1% → 42.3% → **36.9%** of requests, and cost tracks it: $152 → $125 → **$92**. Fold arm best on all axes (35 solved, cheapest, most deferral). No reward difference (32–35/75, p≥0.75). ⚠️ First attempt aborted at 28/75 invalid requests → root-caused **two real `apply` defects** (`62126f4`, `caf32d7`) | ~$380 |
...
| [loca/iter014](loca/iter014/results.md) | 2026-08-23 | **The MERGED design** — coref's criterion inside `extract_llm`'s own bulk call, vs separate components, 128k, n=75/arm | **It declines to act: `merged_keep` 88%, `merged_trim` 1 of 2,074.** Reward indistinguishable (ITT 16 vs 16, p=1.00). Wins on efficiency: **−25% calls, −58% CG spend**. Overcount ratio **31.7×**, the largest measured | ~$460 |
| [loca/iter022](loca/iter022/results.md) | 2026-09-01/02 | Three arms on the shipped `housellm` baseline at 64k: `+evidence,econ_trigger` vs `+coref`. 45 runs, **0 errors, 0 400s** | **Mechanism works (econ 30/30 repaid); the LATENCY claim does not replicate — arm B's turn saving is on the FAILURE path (solved runs 20.1→19.8) at +1.2 s/request. Reward null both ways, harm bound blocks both** | → the band with reward headroom cannot reach the mechanism; choosing the benchmark now outranks choosing the arm |
