...
| `min_batch_frac` | 0.05 | The pass must cut at least this fraction of the request, or it declines and leaves the request byte-identical. Was 0.15, which [measurement showed admits 1 of 19 real sessions](../results/coref-density.md) — a gate no traffic can clear is an off switch, not a conservative default. The right value is an experimental result; this is a starting point. |
