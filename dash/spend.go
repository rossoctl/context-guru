...
//
// Includes cg_llm_cost_usd: context-guru's compaction calls go out on the same
// credential the request carried, so a tenant that enables extract_llm is spending their
// own money through us and the figure has to show it.
//
...
// disk rule — deletes request rows, so a sum over them is a spend figure that silently
// shrinks as history is evicted: 120k requests worth $50 against a 100k-row quota loses
// the oldest 20k rows and under-reports the month. The rollup is incremented in the
// insert transaction and nothing ever deletes from it.
//
// New failure modes, stated plainly:
//
//   - It is MONOTONIC within a month. Deleting rows by hand, or restoring a database,
//     does not refund spend; only the calendar turning over resets it.
...
//     starts the month's total at zero, so the figure under-counts for the rest of that
...
//
// A lookup error returns an error, and every caller treats it as cosmetic: a settings
// page with no spend figure on it is a nuisance, and failing a request over it would not
// be.
