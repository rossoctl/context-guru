...
func init() {
	components.RegisterFields("coref", corefConfig{}, append([]components.Field{
		{Key: "min_tokens", Type: components.FieldInt, Default: 300, Min: 1,
			Hint: "Per-output floor for entering the co-reference index. Matches deploy/harbor/coref.py's min_output so the component and the offline measurement consider the SAME population — change one and the calibration no longer describes the code."},
		{Key: "closed_dist", Type: components.FieldInt, Default: corefClosedDistDefault, Min: 1,
			Hint: "How many messages ago the last literal reference must be before an output counts as CLOSED. A placeholder, not a result: this is one of the two numbers deploy/harbor/coref.py exists to measure on captured traffic, and it has not run on real traffic yet. Only consulted when cut_closed is on."},
		{Key: "open_reps", Type: components.FieldInt, Default: corefOpenRepsDefault, Min: 1,
			Hint: "How many literal references keep an output OPEN regardless of age. The second of the two uncalibrated thresholds; see closed_dist. Only consulted when cut_closed is on."},
		{Key: "min_later_turns", Type: components.FieldInt, Default: corefMinLaterDefault, Min: 0,
			Hint: "OPPORTUNITY FLOOR: an output with fewer of the model's turns after it than this is never cut, because it has not yet had a chance to be referenced — so 'unreferenced' says nothing about it. This is the guard that separates 'nothing reused it' from 'nothing has happened yet', and removing it makes every fresh output look spent."},
		{Key: "cut_unreferenced", Type: components.FieldBool, Default: true,
			Hint: "Cut outputs no later turn literally reuses anything from. ON by default: it is the honest ceiling of a zero-LLM implementation and needs no calibrated threshold. Measured on held-out future as ground truth it removed 11.8% at 95% live-kept — the best discriminator of the ten arms tried, model arms included."},
		{Key: "cut_closed", Type: components.FieldBool, Default: false,
			Hint: "Also cut outputs referenced once or twice, long ago (the 'closed' case). OFF by default because its two thresholds (closed_dist, open_reps) are the OUTPUT of a measurement pass that has not run on real traffic — shipping a guessed closed_dist would ship the one number the design says must be measured. Offline it removed 26.5% but dropped live content 21% of the time against 11%. Turn it on when there are numbers."},
		{Key: "rewrite_budget", Type: components.FieldInt, Default: 3, Min: 0,
			Hint: "Cap on prefix-rewrite passes per session — this component's only cache-spending path. 0 disables new cuts entirely while REPLAY of already-latched decisions continues, which is what keeps a session's earlier saving from being undone. A component that spends cache-writes on purpose has to be answerable for how many."},
		{Key: "min_batch_frac", Type: components.FieldFloat, Default: 0.05, Min: 0,
			Hint: "The pass must cut at least this fraction of the request before a rewrite is worth taking. One rewrite serves every cut in the pass, because a single early cut cannot repay its own rewrite: 5k cut at 20% depth of a 150k transcript needs T > 276 turns, while 60k of the same transcript needs T > 23. The old 0.15 default came from illustrative arithmetic and admitted ONE of nineteen real long sessions (zero at the shipped cut set) — a gate no traffic can clear is an off switch that looks like a threshold. 0.05 admits 16/19, and is a starting point rather than a claim."},
		{Key: "break_even", Type: components.FieldBool, Default: true,
			Hint: "Apply S*T > 11.5*W with an estimated T before spending the rewrite. Shared with extract_llm_sweep's econ trigger rather than restated, so the two components cannot price the same cache-write differently. Ignored when the context window is unknown, like every other fraction-based threshold here: an unresolvable threshold imposes no constraint rather than silently disabling the pass."},
